...
export default function CartItems({items, isEditable}: CartItemsProps) {
  ...
  const totalPrice: number = items.reduce((total, item) => item.amount * item.product.price + total, 0)
        ...
        {items.map((cartItem: CartItem) => (
          <ListItem className={classes.listItem} key={cartItem.product.id}>
            ...
            <Typography variant="body2">
              {formatAsPrice(cartItem.product.price)} x {cartItem.amount} ={' '}
              {formatAsPrice(cartItem.product.price * cartItem.amount)}
            </Typography>
          </ListItem>
        ))}
